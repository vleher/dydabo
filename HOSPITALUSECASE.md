...
This is a use case that I got from somewhere [this blog post](#https://sumitpal.wordpress.com/2016/07/03/hbase-schema-design-example/) and seems to depict a real application scenario. 
...
Unlike relational databases, keeping multiple copies of the same data for easy access is not a bad thing in NoSql databases. We should now create POJO or data beans in the application such that these information can be easily accessed and used by the application.

First thing to consider is the way we will access the data from the back end. These are probably the type of queries that the application will use to data from the database.

1. All the patients in the system
2. Get a patient by patient id
3. All the patients with a specific first name or a last name or both.
5. All the patients with a specific medication (we will ignore diagnosis and procedures in this example, as it will be similar to medication. just to keep it simple)
6. All encounters for a given patient id
7. All encounters for a patient given the first name, or last name or both
8. All encounters with a specific medication id or code (all patients)
9. All encounters with a specific medication id for a patient name with a specific first and last name (or patient id)
10. All medications for a given patient id
11. All medications for all patients
12. Get a claim by claim id
13. All claims for a specific patient id (or by last name)
14. All claim charges for a specific patient id

There are several more queries you can come up with but most of them will be a variation of the above. We will also assume the following in this case

* after an encounter the data will not change
* any subsequent change will be the result of another encounter

Now, we will need to design the data objects such that the data can easily be used by the application. The design below is probably just one of the many you can have. It is more geared towards all the different ways the library can be used to save and access data. Your own design will probably differ depending on your requirements.
...
> **TIP**: You should probably use short variable names as these will be used as column names. A long variable name, while useful for legibility in Java classes can create large disk footprints for table. You can choose something that works for both and keep your variable length to 3 to 5 characters.
...
Before we go ahead with other classes, let quickly take a look at the class that are referenced from the Patient class, such as Address. We have a Map of Addresses for each patient. 
...
The class should be self explanatory. Notice that we do not have to implement the BlackBoxable interface as this class will have its own table. The data will be store as part of the Patient table. 
...
Let's go ahead and create the other classes as well for the different objects. You must have noticed that the emphasis is on the application here and much less the database design. Although you could argue that it closely relates to a relational model at this time. 

Let's go ahead and build our other classes that relates to Medication, Claim, ClaimDetails and ClaimCharges. The Procedure and Diagnosis classes will closely resemble the Medication.
...
````
You now have the choice of keeping the ClaimDetails and ClaimCharges as separate tables or just as part of the claim. Either will work and what you want to do will depend on the requirements. For example,  if you want to very quickly lookup claim charges with out parsing through all the claims, then keeping it as a separate table might help.
...
blackBox.update(claim);

`````

Once the data has been saved to the tables, you will need to access them in various different ways. We will go through the scenarios that we detailed earlier and how we could do each of them. 

#### all patients in the system

If you have millions of patients, then this is probably not a wise idea. But you could do it if you wanted to.

````
// create a bean with all null or empty values and pass it to search for for all patients
...
If you have used just your patient id as the row key on the table, then it is much faster to look up. 
...
So, if you have to look up a patient with either the first or the last name. You can do it in either of the two ways depending on if you have used the name as part of the row key. You can mix and match to search for any fields.
...
In the data model, we kept the medication names as a first level comma separated list for easier search. So, just as with the search we can do a regular expression search on that field.
...
Again, we could iterate over the encounter and get the claim charges. In this example, we will just look through the Claims table and add up the amount.
...
Depending on the type of queries that you would be executing most often, you can modify the design of POJO further to get faster results. Obviously it won't be just as efficient if you were to do it yourself, as you know the semantics of the variables better. A better row key design to search thorough faster would also be a good idea.
