...
/**
 * The BlackBox interface allows you to insert, update, delete, search and fetch objects from the
 * underlying database.
 *
 * @param <T>
...
public interface BlackBox<T extends BlackBoxable> {
	...
	/**
	 * Delete a list of POJO beans that are rows in the table. Each of these objects should have a
	 * valid row key.
	 *
	...
	/**
	 * Get all matching rows given a list of partial keys. This uses the fuzzy filter to search
	 * through keys
	 *
	...
	/**
	 * Get matching rows upto maxResults given a list of partial keys. This uses the fuzzy filter to
	 * search through keys
	 *
	 * @param rows list of partial keys in regex format
	 ...
	 * @throws BlackBoxException blackbox exception
	 */
	List<T> fetchByPartialKey(List<T> rows, int maxResults, boolean isFirst)
	...
	/**
	 * Search for all matching rows in the table, given a list of POJO. You can add known values to
	 * the POJO fields or regular expressions in the field.
	 * <p>
	 * Example: to search for all names starting with David, you can do obj.setName("David.*") and
	 * pass in the obj to the search method.
	 *
	 * @param rows a list of POJO
	...
	/**
	 * Search for all matching rows in the table, given a POJO. You can add known values to the POJO
	 * fields or regular expressions in the field.
	 * <p>
	 * Example: to search for all names starting with David, you can do obj.setName("David.*") and
	 * pass in the obj to the search method.
	 *
	...
	/**
	 * Search for all matching rows in the table, given a two POJOs. You can add known values to the
	 * POJO fields or regular expressions in the field. The two POJO act as the start and end
	...
	/**
	 * Update or insert the rows that match the POJO. If a row does not exist, then it will added
	 * otherwise the existing row is updated with the new values.
	 *
	...
	/**
	 * Update or insert the row that match the POJO. If a row does not exist, then it will added
	 * otherwise the existing row is updated with the new values.
	 *
	 * @param newRow a POJO
	 * @return true if update is successful, false otherwise
