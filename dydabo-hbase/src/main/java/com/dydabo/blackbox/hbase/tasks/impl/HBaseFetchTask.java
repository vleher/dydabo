		...
		try (Table hTable = getConnection().getTable(utils.getTableName(rows.get(0)))) {
			...
			for (Result result : results) {
				if (result.listCells() != null) {
					...
					T resultObject = new Gson().fromJson(resultTable.toJsonObject(),
	...
	private List<T> fetchByPartialKeys(List<T> rows) throws BlackBoxException {
		...
		try (Table hTable = getConnection().getTable(tableName)) {
			for (T row : rows) {
				...
				Filter rowFilter = new RowFilter(CompareOperator.EQUAL,
						new RegexStringComparator(row.getBBRowKey()));
				...
				try (ResultScanner resultScanner = hTable.getScanner(scan)) {
					for (Result result : resultScanner) {
						GenericDBTableRow resultTable =
								utils.parseResultToHTable(result, rows.get(0));
