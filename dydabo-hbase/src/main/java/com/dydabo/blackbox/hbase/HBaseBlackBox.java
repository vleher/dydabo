 ...
 */
public class HBaseBlackBox<T extends BlackBoxable> extends AbstractBlackBoxImpl<T> implements BlackBox<T> {
    ...
    public List<T> fetchByPartialKey(List<T> rows, int maxResults, boolean isFirst) throws BlackBoxException {
        ...
        try (Connection connection = getConnection()) {
            HBaseFetchTask<T> fetchTask = new HBaseFetchTask<>(connection, rows, true, maxResults, isFirst);
            return getForkJoinPool().invoke(fetchTask);
        } catch (IOException ex) {
            logger.error(ex);
        }
    ...
    public List<T> search(List<T> rows, int maxResults, boolean isFirst) throws BlackBoxException {
        ...
        createTable(rows);
        try (Connection connection = getConnection()) {
            HBaseSearchTask<T> searchTask = new HBaseSearchTask<>(connection, rows, maxResults, isFirst);
    ...
    public List<T> search(T startRow, T endRow, int maxResults, boolean isFirst) throws BlackBoxException {
        ...
        if (startRow.getClass().equals(endRow.getClass())) {
            try (Connection connection = getConnection()) {
                HBaseRangeSearchTask<T> searchTask = new HBaseRangeSearchTask<>(connection, startRow, endRow, maxResults, isFirst);
                return getForkJoinPool().invoke(searchTask);
            } catch (IOException ex) {
                logger.error(ex);
            }
