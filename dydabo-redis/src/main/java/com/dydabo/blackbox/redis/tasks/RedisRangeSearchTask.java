...
public class RedisRangeSearchTask<T extends BlackBoxable> extends RedisBaseTask<List<T>, T> {
	...
	public RedisRangeSearchTask(RedisConnectionManager connectionManager, T startRow, T endRow,
	...
	private boolean filter(T rowObject, GenericDBTableRow startTableRow,
			GenericDBTableRow endTableRow) {
		GenericDBTableRow tableRowObject = utils.convertRowToTableRow(rowObject);

		for (Map.Entry<String, GenericDBTableRow.ColumnFamily> familyEntry : tableRowObject
				.getColumnFamilies().entrySet()) {
			GenericDBTableRow.ColumnFamily colFamily = familyEntry.getValue();
			for (Map.Entry<String, GenericDBTableRow.Column> columnEntry : colFamily.getColumns()
					.entrySet()) {
				String columnName = columnEntry.getKey();
				GenericDBTableRow.Column columnValue = columnEntry.getValue();

				GenericDBTableRow.Column startColValue = startTableRow
						.getColumnFamily(colFamily.getFamilyName()).getColumn(columnName);
				GenericDBTableRow.Column endColValue = endTableRow
						.getColumnFamily(colFamily.getFamilyName()).getColumn(columnName);
	...
	// TODO : refactor this
	private boolean compareInRange(GenericDBTableRow.Column startColValue,
			GenericDBTableRow.Column columnValue, GenericDBTableRow.Column endColValue) {
		Object s1 = null;
		Object s2 = null;
		Object s3 = null;

		if (startColValue != null
				&& DyDaBoUtils.isNotBlankOrNull(startColValue.getColumnValueAsString())) {
			s1 = startColValue.getColumnValue();
		}

		if (columnValue != null
				&& DyDaBoUtils.isNotBlankOrNull(columnValue.getColumnValueAsString())) {
			s2 = columnValue.getColumnValue();
		}

		if (endColValue != null
				&& DyDaBoUtils.isNotBlankOrNull(endColValue.getColumnValueAsString())) {
		...
		if (Objects.equals(DyDaBoUtils.EMPTY_ARRAY,
				Objects.requireNonNull(startColValue).getColumnValueAsString())
				&& Objects.equals(DyDaBoUtils.EMPTY_ARRAY,
						Objects.requireNonNull(endColValue).getColumnValueAsString())) {
			flag = true;
		}

		if (Objects.equals(DyDaBoUtils.EMPTY_MAP, startColValue.getColumnValueAsString())
				&& Objects.equals(DyDaBoUtils.EMPTY_MAP,
						Objects.requireNonNull(endColValue).getColumnValueAsString())) {
			flag = true;
		}
