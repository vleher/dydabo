...
public class RedisInsertTask<T extends BlackBoxable> extends RedisBaseTask<Boolean, T> {
	...
	public RedisInsertTask(RedisConnectionManager connectionManager, List<T> rows,
	...
	private Boolean insert(List<T> rows) {
		logger.debug("Inserting {} rows into database. checkExisting:{}", rows.size(),
				checkExisting);
		...
		ForkJoinTask<Boolean> fjTaskOne = new RedisInsertTask<>(getConnectionManager(),
				rows.subList(0, rows.size() / 2), checkExisting);
		taskList.add(fjTaskOne);
		ForkJoinTask<Boolean> fjTaskTwo = new RedisInsertTask<>(getConnectionManager(),
				rows.subList(rows.size() / 2, rows.size()), checkExisting);
		taskList.add(fjTaskTwo);

		return invokeAll(taskList).stream().map(ForkJoinTask::join).reduce(Boolean::logicalAnd)
				.orElse(false);
	}
