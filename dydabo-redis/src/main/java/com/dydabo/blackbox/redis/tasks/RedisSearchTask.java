...
public class RedisSearchTask<T extends BlackBoxable> extends RedisBaseTask<List<T>, T> {
	...
	public RedisSearchTask(RedisConnectionManager connectionManager, List<T> rows, int maxResults,
			boolean isFirst) {
	...
	private List<T> search(List<T> rows) {
		...
		ForkJoinTask<List<T>> fjTaskOne = new RedisSearchTask<>(getConnectionManager(),
				rows.subList(0, rows.size() / 2), maxResults, isFirst);
		taskList.add(fjTaskOne);
		ForkJoinTask<List<T>> fjTaskTwo = new RedisSearchTask<>(getConnectionManager(),
				rows.subList(rows.size() / 2, rows.size()), maxResults, isFirst);
		taskList.add(fjTaskTwo);

		return invokeAll(taskList).stream().map(ForkJoinTask::join).flatMap(Collection::stream)
	...
	private boolean compare(T rowObject, GenericDBTableRow tableRow) {
		...
		for (Map.Entry<String, GenericDBTableRow.ColumnFamily> entry : tableRow.getColumnFamilies()
				.entrySet()) {
			GenericDBTableRow.ColumnFamily colFam = entry.getValue();
			for (Map.Entry<String, GenericDBTableRow.Column> column : colFam.getColumns()
					.entrySet()) {
	...
	private boolean compareColumnStrings(GenericDBTableRow tableRowObject,
			GenericDBTableRow.ColumnFamily colFam, String colName, String colString) {
		if (DyDaBoUtils.isValidRegex(colString)) {
			final String columnValueAsString =
					tableRowObject.getColumnFamily(colFam.getFamilyName()).getColumn(colName)
							.getColumnValueAsString();
			if (colString.startsWith("{") || colString.startsWith("[")) {
				// TODO: compare maps and arrays
			} else {
				Pattern p = Pattern.compile(colString);
				return DyDaBoUtils.isBlankOrNull(columnValueAsString)
						|| !p.matcher(columnValueAsString).matches();
			}
		}
		return false;
	}
