...
public class RedisConnectionManager {
	...
	private static StatefulRedisConnection<String, String> getConnection(String hostName,
			Integer port, Integer database) {
		RedisClient redisClient = redisClientPool.computeIfAbsent(hostName,
				name -> RedisClient.create("redis://" + hostName + ":" + port + "/" + database));
	...
	public void close() {
		RedisConnectionManager.redisClientPool.values().stream().forEach(conn -> {
			conn.close();
			conn.shutdown();
		});
	}
