 ...
 */
public class RedisBlackBox<T extends BlackBoxable> extends AbstractBlackBoxImpl<T> implements BlackBox<T> {
    ...
    public List<T> fetch(List<T> rows) throws BlackBoxException {
        RedisFetchTask<T> fetchJob = new RedisFetchTask<>(getConnectionManager(), rows, false, Integer.MAX_VALUE, false);
    ...
    public List<T> fetchByPartialKey(List<T> rows, int maxResults, boolean isFirst) throws BlackBoxException {
        RedisFetchTask<T> fetchTask = new RedisFetchTask<>(getConnectionManager(), rows, true, maxResults, isFirst);
    ...
    public List<T> search(List<T> rows, int maxResults, boolean isFirst) throws BlackBoxException {
        RedisSearchTask<T> searchTask = new RedisSearchTask<>(getConnectionManager(), rows, maxResults, isFirst);
    ...
    public List<T> search(T startRow, T endRow, int maxResults, boolean isFirst) throws BlackBoxException {
        RedisRangeSearchTask<T> rangeTask = new RedisRangeSearchTask<>(getConnectionManager(), startRow, endRow, maxResults,
                isFirst);
    ...
    public boolean update(List<T> newRows) throws BlackBoxException {
        RedisInsertTask<T> updateTask = new RedisInsertTask<>(getConnectionManager(), newRows, false);
