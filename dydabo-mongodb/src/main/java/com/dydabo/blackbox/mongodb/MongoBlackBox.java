 ...
 */
public class MongoBlackBox<T extends BlackBoxable> extends AbstractBlackBoxImpl<T> implements BlackBox<T> {
    ...
    public List<T> fetch(List<T> rows) throws BlackBoxException {
        MongoFetchTask<T> task = new MongoFetchTask<>(getCollection(), rows, false, Integer.MAX_VALUE, false);
        return task.invoke();
    }

    @Override
    public List<T> fetchByPartialKey(List<T> rows, int maxResults, boolean isFirst) throws BlackBoxException {
        MongoFetchTask<T> task = new MongoFetchTask<>(getCollection(), rows, true, maxResults, isFirst);
        return task.invoke();
    }
    ...
    public List<T> search(T startRow, T endRow, int maxResults, boolean isFirst) throws BlackBoxException {
        MongoRangeSearchTask<T> task = new MongoRangeSearchTask<>(getCollection(), startRow, endRow, maxResults, isFirst);
        return task.invoke();
